...
<div class="row" >
    ...
    <div class="thumbnail" >
        ...
        <?php if($history['hotsearch']){
            ...
            <div class="disk col-xs-3 col-sm-3 col-md-3 col-lg-3">
                <li role="presentation" class="active text-center"><a href="/course/view?search=<?=$v?>"><?=  (mb_strlen($v) > 9) ? mb_substr($v, 0, 6).'...' :$v;?></a></li>
            </div>
        <?php } }else{ ?>
    ...
    <div class="thumbnail">
        ...
        <?php if($history['historysearch']){
            ...
            ?>
            <div class="disk col-xs-3 col-sm-3 col-md-3 col-lg-3">
                <li role="presentation" class="active text-center"><a href="/course/view?search=<?=$v?>"><?= (strlen($v) > 9) ? substr($v, 0, 6).'...' :$v;?></a></li>
            </div>
        <?php } }else{ ?>
...
<script>     
    function link(jump){
        ...
        if(val){
           window.location.href="/course/view?search="+val;
        }else window.location.href="/course/search";    
    }   

    function clearhis(){
         var type="clear" ;
         $.post("/course/clearhis", { type: type },
            function(data){             
               if(data.message>0){
                   alert("清理成功！");
