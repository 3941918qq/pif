...
<script>  
   ...
   function link(jump){
    ...
    var LoadingDataFn=function(){
        ...
        var url='/home/records?page='+page;
        if(type!=''){ url+='&type='+type;}
        if(code!=''){ url+='&code='+code;}
        if(key!=''){url+='&key='+key;}
        if(search!=''){ url+='&search='+search;}
        var box="<div class='loading text-center'><i class='fa fa-spinner fa-spin fa-2x fa-fw'></i><span class='sr-only'>加载中...</span></div>";
        $.ajax({
            type:'get',
            url:url,
            async:false,
            ...
            success:function(ret){           
                $.each(ret.message,function(i,info){                     
                       ...
                       var div="<div class='col-xs-12 col-sm-12 col-md-4 col-lg-4 data-list' style='padding:0;'>\n\
                                     <div class='thumbnail' >\n\
                                    <a href='/home/data-detail?dataid="+info.id+"&unique="+info.unique_key+"'>\n\
