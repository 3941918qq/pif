    ...
    <div class="thumbnail" >
        ...
        <?php if($history['hotsearch']){
            ...
            <div class="disk col-xs-3 col-sm-3 col-md-3 col-lg-3">
                <li role="presentation" class="active text-center"><a href="/success-example/view?search=<?=$v?>"><?= (strlen($v) > 9) ? substr($v, 0, 6).'...' :$v;?></a></li>
            </div>
        <?php } }else{ ?>
...
<script>     
    function link(jump){
        ...
        if(val){
           window.location.href="/success-example/view?search="+val;
        }else window.location.href="/success-example/search";    
    }       

    function clearhis(){
         var type="clear" ;
         $.post("/success-example/clearhis", { type: type },
            function(data){             
               if(data.message>0){
                   alert("清理成功！");
